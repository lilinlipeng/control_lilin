 ...
 */
@Service
public class HealthIndicatorTypeServiceImpl extends ServiceImpl<HealthIndicatorTypeMapper, HealthIndicatorType>
        implements HealthIndicatorTypeService {
     ...
     * 处理健康指标分类对于的外键数据
     */
    private List<HealthIndicatorTypeDto> DispatchItem(List<HealthIndicatorTypeDto> items)
        ...
        for (HealthIndicatorTypeDto item : items) {
            ...
            List<HealthIndicator> HealthIndicatorEntityList = HealthIndicatorMapper.selectList(Wrappers
                    .<HealthIndicator>lambdaQuery().eq(HealthIndicator::getHealthIndicatorTypeId, item.getId()));
            item.setHealthIndicatorDtoList(Extension.copyBeanList(HealthIndicatorEntityList, UserHealthIndicatorDto.class));
    ...
    public PagedResult<HealthIndicatorTypeDto> List(HealthIndicatorTypePagedInput input) {
        ...
        List<HealthIndicatorTypeDto> items = Extension.copyBeanList(pageRecords.getRecords(),
                HealthIndicatorTypeDto.class);
    ...
    public PagedResult<UserHealthIndicatorTypeDto> CommUserHealthIndicatorList(HealthIndicatorTypePagedInput input) {
        ...
        Long totalCount = HealthIndicatorTypeMapper.selectCount(queryWrapper);
        // 把HealthIndicatorType实体转换成HealthIndicatorType传输模型
        List<UserHealthIndicatorTypeDto> items = Extension.copyBeanList(pageRecords.getRecords(),
                UserHealthIndicatorTypeDto.class);

        for (UserHealthIndicatorTypeDto item : items) {
            ...
            List<UserHealthIndicatorDto> HealthIndicatorDtoList = Extension.copyBeanList(HealthIndicatorEntityList,
                    UserHealthIndicatorDto.class);
            for (UserHealthIndicatorDto HealthIndicatorDto : HealthIndicatorDtoList) {
                // 查询当前用户是否存在这个指标
                Long count = HealthIndicatorMapper.selectCount(
                        Wrappers.<HealthIndicator>lambdaQuery()
                                ...
                                .eq(HealthIndicator::getName, HealthIndicatorDto.getName()));

                if (count > 0) {
                    HealthIndicatorDto.setIsSelected(true);
                } else {
                    HealthIndicatorDto.setIsSelected(false);
                }
            }

            item.setHealthIndicatorDtoList(HealthIndicatorDtoList);
