...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改食物类型' : '添加食物类型'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/FoodType/List" :column="columnList">
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/FoodType/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/FoodType/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/FoodType/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
