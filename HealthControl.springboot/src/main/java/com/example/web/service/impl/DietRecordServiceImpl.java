...
public class DietRecordServiceImpl extends ServiceImpl<DietRecordMapper, DietRecord> implements DietRecordService {
   ...
   private List<DietRecordDto> DispatchItem(List<DietRecordDto> items) throws InvocationTargetException, IllegalAccessException {
       ...
       for (DietRecordDto item : items) {
           ...
           //查询出关联的FoodUnit表信息           
            Food  FoodtEntity= FoodMapper.selectById(item.getFoodUnitId());
            item.setFoodDto(FoodEntity!=null?FoodEntity.MapToDto():new FoodDto());
           ...
           item.getFoodUnitDto().setCalories(
              ...
              /*  // 热量 = 食物热量 * 单位值
                ...
                // 糖水化合物 = 食物糖水化合物 * 单位值
               item.getFoodUnitDto().setCarbohydrates(
                        Extension.ToFixed4(item.getFoodUnitDto().getCarbohydrates() * item.getFoodUnitDto().getUnitValue()));
                // 脂肪 = 食物脂肪 * 单位值
               item.getFoodUnitDto().setFat(Extension.ToFixed4(item.getFoodUnitDto().getFat() * item.getFoodUnitDto().getUnitValue()));       
     */   }
