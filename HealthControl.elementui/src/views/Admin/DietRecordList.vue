            ...
            <div class="margin-top-sm">
                <el-form :inline="true" :model="searchForm" size="default">
                    ...
                    <el-form-item label="食物单位">
                        <SigleSelect url="/FoodUnit/List" class="search-input" columnName="UnitName" :clearable="true" columnValue="Id"
                            v-model="searchForm.FoodUnitId">
                        </SigleSelect>
                    </el-form-item>                
...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改饮食记录' : '添加饮食记录'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
            <el-form v-if="editorShow" ref="editModalForm" :rules="editModalFormRules" :model="formData" label-width="140px"
              ...
              <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="记录食物" prop="FoodId">
                    ...
                    <el-col :span="24">
                        <el-form-item label="食物单位" prop="FoodUnitId">
                          <SigleSelect url="/FoodUnit/List" columnName="UnitName" columnValue="Id"  v-model="formData.FoodUnitId" >
                          </SigleSelect>
                        </el-form-item>
                    </el-col>
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/DietRecord/List" :column="columnList">
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/DietRecord/Get`, { Id: Id });
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/DietRecord/CreateOrEdit`, formData);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/DietRecord/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
