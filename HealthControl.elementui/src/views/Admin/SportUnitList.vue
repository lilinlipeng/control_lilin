...
<!-- 编辑对话框 -->
              ...
              <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="单位值" prop="UnitValue">
                            ...
                            <el-input type="number" v-model.number="formData.UnitValue" placeholder="请输入单位值 (1-1000000)"
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/SportUnit/List" :column="columnList">
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/SportUnit/CreateOrEdit`, formData);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/SportUnit/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
