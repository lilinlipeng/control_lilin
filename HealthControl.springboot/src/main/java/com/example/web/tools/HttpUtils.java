...
public   class HttpUtils {
    ...
    public static String Post(String url,Object bodyData, Map<String, String> headerMap) {
        ...
        // 对于 DeepSeek API，需要使用默认的命名策略（保持字段名原样）
        // 如果 bodyData 是 DeepSeekRequestDto，它会使用 @JsonProperty 注解
        // 其他情况使用 UPPER_CAMEL_CASE
        String jsonString;
        if (bodyData != null && bodyData.getClass().getPackage() != null 
            && bodyData.getClass().getPackage().getName().contains("com.example.web.tools.dto")) {
            // DeepSeek DTO 使用默认命名策略，保持字段名原样
            jsonString = mapper.writeValueAsString(bodyData);
        } else {
