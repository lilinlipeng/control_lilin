...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改运动参考' : '添加运动参考'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/Sport/List" :column="columnList">
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/Sport/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
