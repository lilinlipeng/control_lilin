...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改健康知识分类' : '添加健康知识分类'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/HealthArticleType/List" :column="columnList">
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/HealthArticleType/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/HealthArticleType/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/HealthArticleType/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
