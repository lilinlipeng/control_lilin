...
const onEditorReady = () => {
	...
	nextTick(() => {
		// 在 uni-app 组件中，createSelectorQuery 会自动绑定到当前组件
		// 但如果需要显式指定，可以使用 .in(instance.proxy) 或 .in(instance.ctx)
		let query;
		try {
			// 尝试使用 instance.proxy（Vue 3 推荐方式）
			if (instance && instance.proxy) {
				query = uni.createSelectorQuery().in(instance.proxy);
			} else {
				// 如果 proxy 不可用，直接使用 createSelectorQuery
				query = uni.createSelectorQuery();
			}
		} catch (e) {
			// 如果出错，直接使用 createSelectorQuery
			query = uni.createSelectorQuery();
		}
		
		query.select('#editor').context((res) => {
			if (!res || !res.context) {
				// 如果 context 为 null，延迟重试（最多重试5次）
				retryCount++;
				if (retryCount < MAX_RETRY) {
					...
					console.error('编辑器初始化失败：无法获取 editor context');
				}
				return;
			}
	...
	return new Promise((resolve, reject) => {
		editorCtx.value.getContents({
			success: async ({ html, text, delta }) => {
				// 转换 HTTP 为 HTTPS
				const convertedHtml = convertHttpToHttps(html);
				content.value = convertedHtml;
				emit('update:modelValue', convertedHtml);
				emit('contentChange', { html: convertedHtml, text, delta });
...
const convertHttpToHttps = (html) => {
	if (!html) return html;
	// 将 HTML 内容中的 HTTP 图片链接转换为 HTTPS
	// 匹配 img 标签中的 src 属性，以及背景图片等
	return html.replace(/http:\/\/([^"'\s>]+)/gi, (match) => {
		// 只转换图片链接
		if (/\.(jpg|jpeg|png|gif|webp|bmp|svg)(\?|$|"|'|>|\s)/i.test(match)) {
...
const setContent = (text) => {
	...
	// 转换 HTTP 为 HTTPS
	const convertedText = convertHttpToHttps(text);
